...
export const updateProfile = createAsyncThunk(
  ...
  async (userData, { rejectWithValue }) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.errors?.[0] || 'Erro na atualização');
      }
      
      // 3. IMPORTANTE: Preserva o token na resposta
      const updatedUser = {
        ...data.user, // ou data se não tiver .user
        token: currentToken // MANTÉM O TOKEN
      };
      
      console.log('💾 User atualizado com token:', updatedUser);
...
export const { resetMessage, loginSuccess, logout, saveToken } = userSlice.actions;
